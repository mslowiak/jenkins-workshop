pipeline {
    environment{
        ...
        URL_CUSTOM = "https://sabre--tncommdp3.cs8.my.salesforce.com/services"
    }
    agent {
        docker {
        ...
        stage('Deploy'){
            ...
            steps{
                dir('simple-backend/target') {
                    ...
                    --salesforce.username=$env.USERNAME \
                    --salesforce.password=$env.PASSWORD \
                    --salesforce.clientId=$env.CLIENT_ID \
                    --salesforce.clientSecret=$env.CLIENT_SECRET \
                    --salesforce.url=$env.URL_CUSTOM"
                }
            }
        }
        stage('Run'){
